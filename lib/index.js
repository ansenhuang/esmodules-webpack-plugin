...
class ESModulesWebpackPlugin {
  ...
  injectScriptsByHtmlPlugin(data, compilation, esmCompilation) {
    ...
    if (moduleInjectIndex !== -1) {
      ...
      data[bodyKey] = [].concat(
        data[bodyKey].slice(0, moduleInjectIndex),
        moduleScripts,
        data[bodyKey].slice(moduleInjectIndex)
      );
      return;
    }
